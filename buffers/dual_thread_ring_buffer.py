...
class RingBufferDualThread:
    ...
    def wait_for_free_space(self, required_space=1):
        required_space = min(required_space, self.capacity)

        with self.condition:
            while self.capacity - self.size < required_space and not self.closed:
                self.condition.wait()
